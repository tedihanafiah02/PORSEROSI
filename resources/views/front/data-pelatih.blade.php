...
@section('title', (app()->getLocale() === 'en' ? 'Coach Directory' : 'Direktori Pelatih') . ' | PB PORSEROSI')
...
@section('content')
        ...
        <div class="relative pt-32 pb-12 md:pt-40 md:pb-16 text-center overflow-hidden">
            ...
            <div class="relative z-10 px-4 max-w-4xl mx-auto">
                ...
                <h1 class="text-3xl md:text-5xl font-extrabold text-white mb-4 uppercase tracking-tight">
                    {{ app()->getLocale() === 'en' ? 'Official Coaches' : 'Pelatih Resmi' }}
                </h1>
                <p class="text-slate-400 text-sm md:text-base max-w-2xl mx-auto leading-relaxed">
                    {{ app()->getLocale() === 'en' 
                        ? 'Directory of certified national and international coaches registered under PB PORSEROSI.' 
                        : 'Direktori pelatih berlisensi daerah, nasional, dan internasional yang terdaftar resmi di bawah PB PORSEROSI.' 
                    }}
                </p>
            </div>
        </div>
                ...
                @if($pelatihs->isEmpty())
                    <div class="text-center py-20 bg-[#181836]/40 backdrop-blur-md border border-white/5 rounded-3xl p-8 max-w-lg mx-auto" data-aos="fade-up">
                        ...
                        <p class="text-slate-400 text-sm">
                            {{ app()->getLocale() === 'en' ? 'No coach data available.' : 'Data pelatih belum tersedia.' }}
                        </p>
                    </div>
                @else
                    {{-- Search Input --}}
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="border-b border-white/5 bg-white/5">
                                        ...
                                        <th class="p-5 text-xs font-bold uppercase tracking-wider text-slate-400">Klub Asal</th>
                                ...
                                <tbody id="pelatih-table-body" class="divide-y divide-white/5">
                                    @foreach($pelatihs as $pelatih)
                                        <tr class="pelatih-row hover:bg-white/5 transition-colors duration-200" 
                                            data-searchable="{{ strtolower($pelatih->nama_lengkap) }} {{ strtolower($pelatih->klub_asal) }} {{ strtolower($pelatih->kabupaten_kota) }} {{ strtolower($pelatih->provinsi) }} {{ strtolower($pelatih->disiplin) }} {{ strtolower($pelatih->lisensi) }}">
                                            
                                            {{-- Foto --}}
                                            <td class="p-5">
                                                <div class="w-10 h-10 rounded-full overflow-hidden border border-white/10 shrink-0">
                                                    @if($pelatih->foto_path && $pelatih->foto_path !== 'test-dummy.png')
                                                        <img src="{{ Storage::url($pelatih->foto_path) }}" alt="{{ $pelatih->nama_lengkap }}" class="w-full h-full object-cover">
                                                    @else
                                                        ...
                                                        <div class="w-full h-full bg-yellow-500/10 flex items-center justify-center text-yellow-500 font-extrabold text-sm uppercase">
                                                            {{ substr($pelatih->nama_lengkap, 0, 2) }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            <td class="p-5 text-sm">
                                                @php
                                                    ...
                                                    $colorClass = $lisensiColors[$pelatih->lisensi] ?? 'bg-slate-500/10 border-slate-500/20 text-slate-300';
                                                @endphp
                                            ...
                                            {{-- Klub Asal --}}
                                            <td class="p-5 text-white font-semibold text-sm">
                                                {{ $pelatih->klub_asal }}
                                            </td>

                                            {{-- Kota / Kab --}}
                                            <td class="p-5 text-slate-300 text-sm">
                                                {{ $pelatih->kabupaten_kota }}
                                            </td>
                                            ...
                                            <td class="p-5 text-center">
                                                @if($pelatih->disiplin)
                                                    @php
                                                        $discList = array_map('trim', explode(',', $pelatih->disiplin));
                                                    @endphp
                    ...
                    <div id="no-search-results" class="hidden text-center py-20 bg-[#181836]/40 backdrop-blur-md border border-white/5 rounded-3xl p-8 max-w-lg mx-auto mt-6" data-aos="fade-up">
                        ...
                        <p class="text-slate-400 text-sm">
                            {{ app()->getLocale() === 'en' ? 'No coaches match your search.' : 'Tidak ada pelatih yang cocok dengan pencarian Anda.' }}
                        </p>
                    </div>
                @endif
    ...
    {{-- JS Search --}}
    <script>
        function searchPelatih() {
            const query = document.getElementById('search-pelatih').value.toLowerCase().trim();
